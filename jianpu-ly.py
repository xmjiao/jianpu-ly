...
def lilypond_command():
    if hasattr(shutil, 'which'):
        ...
        placesToTry = ['/Applications/LilyPond-2.22.2.app/Contents/Resources/bin/lilypond',
                       '/Applications/LilyPond-2.20.0.app/Contents/Resources/bin/lilypond'] + placesToTry
        ...
        placesToTry += ['lilypond-2.24.0/bin/lilypond',
...
def score_end(**headers):
    ...
    if midi:
        ...
        ret += r'\midi { \context { \Score midiInstrument = "Piano" tempoWholesPerMinute = #(ly:make-moment 84 4)}}'
...
def scoreError(msg, word, line):
    ...
    if not word in line:
        ...
        msg += "\n"+re.sub(r"(\s|^)"+re.escape(word)+r"(?=\s|$)",
                           lambda m: m.group(1)+"\x1b[4m"+word+"\x1b[m", line)
    elif re.match('[ -~]*$', line):  # all ASCII: we can underline the word with ^^s
        msg += "\n"+line+"\n"+re.sub('[^^]', ' ', re.sub(r"(\s|^)"+re.escape(
            word)+r"(?=\s|$)", lambda m: m.group(1)+'^'*(len(word)), line))
...
class NoteheadMarkup:
    ...
    def endScore(self):
        if self.barPos == self.startBarPos:
            ...
            errExit("Score %d should end with a %g-beat bar to make up for the %g-beat anacrusis bar.  Set j2ly_sloppy_bars environment variable if you really want to break this rule." %
                    (scoreNo, self.startBarPos/self.beatLength, (self.barLength-self.startBarPos)/self.beatLength))
        else:
            errExit("Incomplete bar at end of score %d (pos %d)" %
                    (scoreNo, self.barPos))
    ...
    def wholeBarRestLen(self): return {96: "1.", 48: "2.", 32: "2", 24: "4.", 16: "4", 12: "8.", 8: "8"}.get(
        self.barLength, "1")  # TODO: what if irregular?
    ...
    def __call__(self, figures, nBeams, dots, octave, accidental, tremolo, word, line):
        ...
        if len(figures) > 1:
            if accidental:
                # see TODOs below
                scoreError(
                    "Accidentals in chords not yet implemented:", word, line)
            if '0' in figures:
...
"""  # TODO: can do accidentals e.g. #:halign 1 #:line ((#:fontsize -5 (#:raise 0.7 (#:flat))) (#:bold "3")) but might cause the beam not to extend its full length if this chord occurs at the end of a beamed group, + accidentals won't be tracked by Lilypond and would have be taken care of by jianpu-ly (which might mean if any chord has an accidental on one of its notes we'd have to do all notes in that bar like this, whether they are chords or not)
        ...
        if placeholder_chord.startswith("<"):
            ...
            if not placeholder_chord == "r":
                ret += {"": "'", "'": "''", "''": "'''", ",": "", ",,": ","}[
                    octave]  # for MIDI + Western, put it so no-mark starts near middle C
        ret += ("%d" % length) + dots
        if tremolo:
            if lilypond_minor_version() < 20:
                errExit("tremolo requires Lilypond 2.20+, we found 2." +
            ...
            if midi or western:
                if placeholder_chord.startswith("<") and len(placeholder_chord.split()) == 4:
                    ...
                    ret = r"%s\repeat tremolo %d { %s32 %s32 }" % (
                        previous, int(toAdd_preTuplet/4), n1, n2)
                else:
                    ret += tremolo
        ...
        self.unicode_approx.append(u""+(u"-" if invisTieLast else figures[-1:])+(u"" if invisTieLast else (u'\u0323' if "," in octave else u'\u0307' if "'" in octave else u""))+beamC+u''.join(
            c+beamC for c in dots)+(u"" if self.inBeamGroup else u" "))  # (NB inBeamGroup is correct only if not midi and not western)
        if self.barPos == self.barLength:
...
def parseNote(word, origWord, line):
    ...
    if re.match(r"[\\]+$", nBeams):
        nBeams = len(nBeams)  # requested by a user who found British note-length names hard to remember; won't work if the \ is placed at the start, as that'll be a Lilypond command, so to save confusion we won't put this in the docstring
    elif nBeams:
        try:
            nBeams = list("cqsdh").index(nBeams)
        except ValueError:
            scoreError("Can't calculate number of beams from " +
                       nBeams+" in", origWord, line)
    else:
    ...
    for line in __doc__.split("\n"):
        ...
        if ":" in line and line.split(":", 1)[1].strip():
            ...
            if re.match(r".*[A-Za-z]\)$", shouldType):
                ...
                note = ""
            if "--html" in sys.argv:
                print("<tr><td>"+htmlify(toGet.strip())+"</td><td><kbd>" +
                      htmlify(shouldType.strip())+"</kbd>"+htmlify(note)+"</td>")
            else:
            ...
            for word in line.split():
                ...
                if word.startswith('%'):
                    ...
                    finger = {
                        ...
                        "3": u"\u4e09", "4": u"\u56db",
                        "souyin": u"\u4e45",  # jiu3
                        # white circle: TODO: can we use Lilypond's ^\flageolet command (not in a \finger{}) which doesn't require a font with 25CB in it? or would that get wrong size? (can be tweaked)
                        "harmonic": u"\u25cb",
                        "up": u"\u2197",  # NE arrow
                        "down": u"\u2198",  # SE arrow
                ...
                # TODO: document this.  If this is on, you have to use c in a note to go back to crotchets.
                    ...
                    notehead_markup.barPos = (
                        notehead_markup.barPos + (newBarPos-oldBarPos)*extraRepeats) % notehead_markup.barLength
   ...
   (#:override
       ...
       (list (list (quote moveto) 0 0)
             ...
             (list (quote curveto) (* textWidth 0.5) -1 (* textWidth 0.5) -1 0 -1)))))))))))) """ + out[lastPtr]
                    ...
                    if not_angka and "'" in octave:
                        maxBeams = max(maxBeams, len(octave)*.8+nBeams)
                    else:
    ...
    if midi or western:
        for numNotes, dot, result in [
            ...
            out = re.sub("(?P<note>[^<][^ ]*|<[^>]*>)4"+dot+r"((?::32)?) +~(( \\[^ ]+)*) " +
                         " +~ ".join(["(?P=note)4"+dot]*(numNotes-1)), r"\g<1>"+result+r"\g<2>\g<3>", out)
            if dot:
                chkLen = 6
            else:
                chkLen = 4
            out = re.sub(r"\\repeat tremolo "+str(chkLen)+r" { (?P<note1>[^ ]+)32 (?P<note2>[^ ]+)32 } +~(( \\[^ ]+)*) "+" +~ ".join(
                ["< (?P=note1) (?P=note2) >4"+dot]*(numNotes-1)), r"\\repeat tremolo "+str(chkLen*numNotes)+" { \g<1>32 \g<2>32 }\g<3>", out)
            out = out.replace(" ".join(["r4"+dot]*numNotes), "r"+result)
        # dynamics need to attach inside the tremolo (but \bar doesn't)
        out = re.sub(
            r"(\\repeat tremolo [^{]+{ [^ ]+)( [^}]+ })(( +\\[^b][^ ]*)+)", r"\g<1>\g<3>\g<2>", out)
        out = re.sub(
            r"(%\{ bar [0-9]*: %\} )r([^ ]* \\bar)", r"\g<1>R\g<2>", out)
...
def process_input(inDat):
    ...
    if unicode_mode:
        return get_unicode_approx(re.sub(r"\sUnicode\s", " ", " "+inDat+" ").strip())
    ret = []
    ...
    for score in re.split(r"\sNextScore\s", " "+inDat+" "):
        ...
        parts = [p for p in re.split(
            r"\sNextPart\s", " "+score+" ") if p.strip()]
                ...
                for partNo, part in enumerate(parts):
                    ...
                    if midi:
                        ...
                        if notehead_markup.withStaff:
                            ...
                            ret.append(staffStart+" "+getLY(part)
                                       [0]+" "+western_staff_end())
                            western = False
                        if lyrics:
                            ret.append("".join(lyrics_start(
                                voiceName)+l+" "+lyrics_end()+" " for l in lyrics))
